...
function seven(m) {
  // Use recursion
  //  Num to string, split to chars, pop last char
  //  Make strs back to nums, Do math
  //  Not done && Len > 2?
  //    Do it again
  // Count the steps
  ...
  while (num > 99) {
    steps += 1;
    let front = Number(String(num).slice(0, -1));
    let last = Number(String(num).slice(-1));
    let n = front - 2 * last;
    console.log(n);
    num = n;
  }
...
function between(a, b) {
  // loop through, pushing to new arr
